...
spleen_score <- spleenscore |>
  ...
  annotate("text", x = "Early", y = 2.9, label = "italic(p) == 0.021", parse = T, size = 5) +
  ggtitle("") +
  xlab("") +
...
qPCR <- pcr |>
  ...
  annotate("text", x = "Early", y = 8.9, label = "italic(p) == 0.091", parse = T, size = 5) +
  ggtitle("") +
  xlab("")+
...
corr <- sc |>
  ...
  scale_y_continuous(name = "CFU equivalents (logqPCR)") +
  labs(title = "") +
  theme_classic(base_size = 20)
corr
...
gran <- liver |>
  ...
  annotate("text", x = "Early", y = 78000, label = "italic(p) == 0.96", parse = T, size = 5) +
  ggtitle("") +
  xlab("") +
...
patindex_tp |>
  ...
  geom_point(show.legend = FALSE, shape = 19, color = "grey30", size = 1.5) +
  annotate("text", x = "Early", y = 9.9, label = "italic(p) == 0.01", parse = T, size = 5)+
  expand_limits(y=0) +
  ggtitle("") +
  xlab("")+
